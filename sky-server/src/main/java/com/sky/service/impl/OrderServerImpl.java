...
public class OrderServerImpl implements OrderService {
    ...
    public OrderPaymentVO payment(OrdersPaymentDTO ordersPaymentDTO) throws Exception {
        ...
        /*
        因为跳过了订单支付，导致小程序端只会访问user/order/payment
        而跳过访问notify/paySuccess,
        故手动调用paySuccess,通过websocket向客户端浏览器推送消息
        */
    ...
    public PageResult historyQuery(OrdersPageQueryDTO ordersPageQueryDTO) {
        //查询所有订单信息，用pageHelper初始化
        PageHelper.startPage(ordersPageQueryDTO.getPage(), ordersPageQueryDTO.getPageSize());
        ordersPageQueryDTO.setUserId(BaseContext.getCurrentId());

        //得到返回的所有订单数据
        Page<Orders> page = orderMapper.historyQuery(ordersPageQueryDTO);
        List<OrderVO> list = new ArrayList<>();

        //对订单数据遍历
        if (page != null) {
            for (Orders orders : page) {
                //根据得到的1个订单数据，查询n个订单详细
